...
class DeadlineEditPresenter(
    ...
    override fun attach(view: EventAddContract.View?) {
        super.attach(view)
        view?.setDeadlinesVisibilist(true)

        schedulerId?.let {
            if (viewModel != null) {
                view?.setContent(viewModel!!)
                return@let
            }

            SchedulersRepository.shared.getSchedulerItemById(schedulerId) { schedulerModel ->
                viewModel = AddEventFieldsViewModel().apply {
                    name
                    auditory = schedulerModel?.auditorium?.firstOrNull()?.name
                    teacher = schedulerModel?.teachers?.firstOrNull()?.name
                    schedule = schedulerModel?.name
                    notes
                    date
                    timeBegin
                    timeEnd
                ...
                schedulerModel?.teachers?.firstOrNull()?.let {
                    TeachersRepository.shared.getByID(it.id.toString()) {
                        selectedTeacher = it
                    }
                }

                schedulerModel?.auditorium?.firstOrNull()?.let {
                    selectedAuditory = it
                }
                selectedScheduler = schedulerModel
    ...
    override fun saveClicked() {
        ...
        val model = EventModel().apply {
            ...
            course = selectedScheduler?.course
    ...
    private fun checkModelValid(viewModel: AddEventFieldsViewModel): Boolean {
        ...
        if (viewModel.teacher.isNullOrEmpty() && false) {
            ++errors
            view?.setTeacherError(R.string.fill_field)
        }

        if (viewModel.schedule.isNullOrEmpty()) {
            ++errors
            view?.setScheduleError(R.string.fill_field)
        }
